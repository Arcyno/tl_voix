...
portSerie ouvrePortSerieBloquant(char * nom, int vitesse, int bits, int parite, int stops, int bloque) {
	...
	h->windowsHandle = CreateFile(
		nom, 								// Port sÃ©rie
		GENERIC_READ+GENERIC_WRITE,			// En lecture et Ã©criture
		0,									// que pour moi.
		NULL,
		OPEN_EXISTING,						// Ce port existe forcÃ©ment
	...
	// Le thread de lecture doit impÃ©rativement rendre	
	// la main le plus souvent possible, car il n'est
	// pas possible de lire et d'Ã©crire simultanÃ©ment
...
int lecturePortSerie(portSerie h, char * c, int len) {
	...
	while (totalBytesRead < len) {
		// Le readfile rend la main au bout d'un timeout si pas de caractÃ¨re
		// reÃ§u.
		ReadFile(
		...
		// Si le nombre de caractÃ¨re n'a pas Ã©tÃ© reÃ§u, on
		// attend une milliseconde avant de relance une
		// lecture. Cela donne l'occasion Ã  un autre thread
		// de pouvoir Ã©mettre.
		if (totalBytesRead < len) {
			Sleep(1);
			if (((_serialHandle_)h)->bloque == 0) {
				// En mode non bloquant, nous venons d'attendre
				// 1 ms. VÃ©rifions si des caractÃ¨res ont Ã©tÃ©
				// reÃ§us dans l'intervalle, auquel cas, on les
				// rÃ©cupÃ¨re.
				ReadFile(
					((_serialHandle_)h)->windowsHandle, 
					c + totalBytesRead, 
					...
					NULL);
				totalBytesRead += NumberOfBytesRead;
				// Renvoyons le nombre de caractÃ¨res reÃ§us :
